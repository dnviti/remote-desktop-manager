...
import { Business, CheckCircle, Fingerprint } from "@mui/icons-material";
...
export default function LoginPage() {
  ...
  const [pendingLoginData, setPendingLoginData] =
    useState<AuthSuccessResponse | null>(null);
  ...
  const buildRedirect = () => {
    ...
    return autoconnect
      ? `/?autoconnect=${encodeURIComponent(autoconnect)}`
      : "/";
  };
  ...
  const completeLogin = (data: AuthSuccessResponse) => {
    ...
    if (acceptedMemberships.length >= 2) {
      ...
      const preselect =
        acceptedMemberships.find((m) => m.tenantId === lastId) ??
    ...
    const activeMembership =
      memberships.find((m) => m.isActive) ?? acceptedMemberships[0];
    if (activeMembership) {
      useUiPreferencesStore
        .getState()
        .set("lastActiveTenantId", activeMembership.tenantId);
    }
  ...
  const handleTenantConfirm = async () => {
    ...
    try {
      ...
      useUiPreferencesStore
        .getState()
        .set("lastActiveTenantId", selectedTenantId);
      navigate(buildRedirect());
    } catch (err: unknown) {
  ...
  const applyAuthResponse = async (
    data: AuthSuccessResponse | Awaited<ReturnType<typeof loginApi>>,
  ) => {
  ...
  const handlePasskeyAuth = async () => {
    ...
    try {
      ...
      if ((err as Error)?.name === "NotAllowedError") {
        ...
        registerPasskeyFailure(
          extractApiError(err, "Passkey authentication failed."),
        );
      }
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- auto-attempt once when the login flow enters passkey mode
  ...
  const subtitle = (() => {
    switch (step) {
      ...
      case "credentials":
        return "Sign in to manage your connections";
      case "mfa-choice":
      ...
      case "webauthn":
        return "Verify your identity with your security key or passkey";
      case "mfa-setup":
        return "Your organization requires two-factor authentication";
      case "tenant-select":
        return "Select the organization you want to work in";
    }
  })();
  ...
  return (
    <Box
      sx={{
        ...
        background: (theme) =>
          `radial-gradient(ellipse at 50% 40%, ${theme.palette.primary.main}0A 0%, ${theme.palette.background.default} 70%)`,
      }}
    >
      <Box
        sx={{
        ...
        {step === "passkey" && (
          <Box>
            ...
            <Button
              fullWidth
              variant="text"
              onClick={handleUsePasswordFallback}
              sx={{
                mb: 1,
                color: "text.secondary",
        ...
        {step === "credentials" && (
          <Box component="form" onSubmit={handleSubmit}>
            ...
            {ldapEnabled && (
              ...
              >
                {ldapProviderName} directory login is available. Use your
                directory credentials above.
              </Typography>
            )}
            {passkeySupported && (
              <Button
        ...
        {step === "email" && (
          <Box component="form" onSubmit={handleEmailSubmit}>
            ...
            <Button
              ...
              variant="text"
              onClick={
                canGoBackToChoice ? handleBackToChoice : handleBackToCredentials
              }
              sx={{
                mb: 1,
                color: "text.secondary",
              ...
              <Box component="form" onSubmit={handleMfaSetupSubmit}>
                <Alert
                  ...
                  sx={{
                    ...
                    color: "warning.light",
                    border: (theme) =>
                      `1px solid ${theme.palette.warning.main}26`,
                    "& .MuiAlert-icon": { color: "warning.light" },
                  }}
                >
                  Your organization requires MFA. Set up an authenticator app to
                  continue signing in.
                </Alert>
                <Typography
                  variant="body2"
