...
Contextual credential autofill detects login forms on web pages and offers to fill username and password fields with matching credentials from the Arsenale keychain. The content script runs at document_idle on all pages: it scans the DOM for password inputs and nearby username/email fields, watches for dynamically added forms via MutationObserver (for SPAs), and injects a floating key icon near each detected password field using shadow DOM to isolate from host page styles and CSP. Clicking the icon opens a dropdown listing matching LOGIN-type credentials matched by URL prefix or domain against the current page. On selection, the full secret is fetched via the service worker and form fields are filled using the native input value setter to trigger framework change detection. The service worker maintains a lightweight in-memory credential index (secretId, name, url, domain) built from the listSecrets API response, refreshed every 5 minutes and on vault unlock. The extension badge displays the number of matching credentials for the active tab. If the vault is locked, the dropdown shows a prompt to unlock it via the extension popup. Autofill preferences (global enable/disable, per-site disable list) are stored in chrome.storage.local and configurable from the extension options page.
